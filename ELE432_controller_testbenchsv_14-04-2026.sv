...
module testbench();
  ...
  function automatic bit matches_expected(input logic [15:0] actual_value,
                                          input logic [15:0] expected_value);
    for (int i = 0; i < 16; i = i + 1)
      if (expected_value[i] !== 1'bx && actual_value[i] !== expected_value[i])
        return 0;
    return 1;
  endfunction
  ...
  initial
    begin
      if (!$value$plusargs("TV=%s", vectorfile))
        vectorfile = "C:/Users/furka/Projects/ELE432_MulticycleRiscv/controller.tv";

      vector_fd = $fopen(vectorfile, "r");
      if (vector_fd == 0)
        $fatal(1, "Could not open test vector file: %s", vectorfile);
      $fclose(vector_fd);
      $readmemb(vectorfile, testvectors);
  ...
  always @(negedge clk)
    if (~reset) begin // skip cycles during reset
      ...
      if (!matches_expected(actual, expected)) begin  // check result, treating X in expected as don't-care
